...
** Last update Tue Jan 26 22:39:35 2016 Sebastien BRUERE
*/

#include	<lapin.h>
#include	"../include/my.h"

void		my_supy(t_bunny_pixelarray *pix,
...
void		my_supx(t_bunny_pixelarray *pix,
			t_bunny_position *pos, t_color *color,
			int a)
{
  int		b;
...
void		my_equaly(t_bunny_pixelarray *pix,
...
void		my_equalx(t_bunny_pixelarray *pix,
			  t_bunny_position *pos,
			  t_color *color, int y)
{
  int		a;
			...
			t_bunny_position *pos, t_color *color)
{
  ...
  savey1 = y;
  if (pos[1].x == pos[0].x)
    my_equalx(pix, pos, color, y);
  else if (pos[1].y == pos[0].y)
    my_equaly(pix, pos, color, x);
